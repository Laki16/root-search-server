...
namespace ApiServer
{
	public class ResultManager
	{
		...
		/// <returns>커넥션이 정상적으로 등록되지 않았다면 null</returns>
		public bool TryAddConnection(
			string secret, string reqKeyword, HttpResponse response, out ClientConnection connection)
		{
			// 중복된 키로 요청이 들어오면 이전 커넥션 캔슬 (자동으로 Dispose 될 것)
			if (connections.TryRemove(secret, out var prevConnection))
			{
